// ─── Shared LLM client, CORS, validation, rate limiting, and prompt helpers ──
...
function getProvider() {
  ...
  // Support GROQ_KEY as legacy fallback
...
export function setSecurityHeaders(res) {
  ...
  res.setHeader(
    ...
    "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://*.tile.openstreetmap.org; connect-src 'self' http://localhost:* https://api.groq.com https://api.openai.com https://api.anthropic.com https://generativelanguage.googleapis.com"
  )
}
...
export async function callGroq({ systemPrompt, userPrompt, maxTokens = 400 }) {
  ...
  try {
    ...
    if (provider.format === 'anthropic') {
      ...
      // openai format — works for Groq, OpenAI, Together, Fireworks, Mistral, etc.
...
// ─── JSON parsing from LLM output (#10 — robust fallback) ────────────────────
...
export function parseLLMJson(text) {
  try {
    ...
    // Fallback: return the raw text wrapped in a sensible structure
...
export function safeError(res, e, context = 'API') {
  ...
  // Set status after json to avoid double-send edge cases
  res.status(status)
}
